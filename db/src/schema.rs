...
mod test {
    ...
    fn invalid_schema_unique_value_not_index() {
        ...
        match err.unwrap() {
            Error(DbError::BadSchemaAssertion(message), _) => { assert_eq!(message, ":db/unique :db/unique_value without :db/index true for entid: :foo/bar"); },
    ...
    fn invalid_schema_unique_identity_not_index() {
        ...
        match err.unwrap() {
            Error(DbError::BadSchemaAssertion(message), _) => { assert_eq!(message, ":db/unique :db/unique_identity without :db/index true for entid: :foo/bar"); },
            x => panic!("expected Bad Schema Assertion error, got {:?}", x),
        }
    }
    ...
    fn invalid_schema_component_not_ref() {
        ...
        match err.unwrap() {
            Error(DbError::BadSchemaAssertion(message), _) => { assert_eq!(message, ":db/isComponent true without :db/valueType :db.type/ref for entid: :foo/bar"); },
            x => panic!("expected Bad Schema Assertion error, got {:?}", x),
        }
    }
    ...
    fn invalid_schema_fulltext_not_index() {
        ...
        match err.unwrap() {
            Error(DbError::BadSchemaAssertion(message), _) => { assert_eq!(message, ":db/fulltext true without :db/index true for entid: :foo/bar"); },
            x => panic!("expected Bad Schema Assertion error, got {:?}", x),
        }
    }
    ...
    fn invalid_schema_fulltext_index_not_string() {
        ...
        match err.unwrap() {
            Error(DbError::BadSchemaAssertion(message), _) => { assert_eq!(message, ":db/fulltext true without :db/valueType :db.type/string for entid: :foo/bar"); },
            x => panic!("expected Bad Schema Assertion error, got {:?}", x),
        }
    }
}
