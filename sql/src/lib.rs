...
impl QueryBuilder for SQLiteQueryBuilder {
    ...
    fn push_bind_param(&mut self, name: &str) -> EmptyBuildQueryResult {
        ...
        if !name.chars().all(|c| char::is_alphanumeric(c) || c == '_') {
            return Err(MentatSqlError::InvalidParameterName(name.to_string()))
        }

        if name.starts_with(self.arg_prefix.as_str()) &&
           name.chars().skip(self.arg_prefix.len()).all(char::is_numeric) {
               return Err(MentatSqlError::BindParamCouldBeGenerated(name.to_string()))
        }
